...
function allocateTaxByPrice(prices: number[], taxRatePct: number) {
  ...
  if (subtotalCents === 0) {
    // If all prices are 0, split tax evenly (or all zero)
...
export default function ShoppingPage() {
  ...
  const openCheckoff = (it: ItemRow) => {
    ...
    setTaxRate(""); // user enters either Food Tax % or Sales Tax % depending on category
    setReceiptFile(null);
  };
  ...
  const confirmPurchaseSingle = async () => {
    ...
    // category decides which tax bucket this belongs to
    ...
    const taxTotal =
      priceNum === null
        ? null
        : round2(subtotal * ((isFood ? foodRate : salesRate) / 100));
    const total = priceNum === null ? null : round2(subtotal + (taxTotal ?? 0));

    // Create a receipt row even for single item
  ...
  const selectedItems = useMemo(() => {
    ...
    // keep list order (same as on screen)
  ...
  const receiptComputed = useMemo(() => {
    ...
    // Parse prices for each selected item (require all)
    ...
    for (const it of selectedItems) {
      ...
      else nonFoodIds.push(it.id); // treat everything else as non-food
    }
    ...
    // Build per-item tax map aligned to selectedItems order
    ...
    const subtotal = round2(
      selectedItems.reduce((sum, it) => sum + pricesById[it.id], 0)
    );
  ...
  const confirmAttachReceipt = async () => {
    ...
    if (!computed) {
      return alert("Enter valid prices for each selected item and valid tax rates.");
    }
          ...
          return (
            <li
              ...
              onClick={() => {
                ...
                if (it.status === "purchased") return; // only select open items
                toggleSelect(it.id);
              }}
            >
